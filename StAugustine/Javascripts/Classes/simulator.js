...
* @type {string}
*/
DDSimulator.MODEL_URL = 'http://hexa.fiu.edu/DPanther_Server/data/3DModel/smart.kmz';
//                        'http://hexa.fiu.edu/DPanther_Server/data/3DModel/BiltmoreHotel.kmz',
//                        'http://hexa.fiu.edu/DPanther_Server/data/3DModel/BiltmoreHotel.kmz'];
...
function DDSimulator(ge, path, opt_opts) {
  ...
  this.currentLoc = this.path[0].loc;
   // this.currentLoc = this.path.steps[0].start_location;
   // this.currentLoc = this.path[0];
    // private vars
...
DDSimulator.prototype.initUI = function (opt_cb) {
    var me = this;
//    for (var i = 0; i < DDSimulator.MODEL_URL.length; i++) {
...
DDSimulator.prototype.tick_ = function () {
    ...
    if (this.pathIndex_ >= this.path.length - 1) {
        this.doTick_ = false;
        //VTMain.methods.TourFinished();
        VT.methods.TourFinished();
        return;
    }
    ...
    my_heading = this.geHelpers_.getHeading(this.path[this.pathIndex_].loc, this.path[this.pathIndex_ + 1].loc);
    my_lat = this.currentLoc.lat();
    my_lng = this.currentLoc.lng();

    //CheckPopup(my_lat, my_lng);
