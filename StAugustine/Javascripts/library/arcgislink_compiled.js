...
(function () {
    ...
    b) { if (r(b)) for (var c, d = 0, e = b.length; d < e; d++) { c = b[d]; if (r(c)) z.addToMap(a, c); else V(c) && c.setMap(a) } }; z.removeFromMap = function (a, b) { z.addToMap(null, a); if (b) a.length = 0 }; m.prototype.forward = function (a) { return a }; m.prototype.inverse = function (a) { return a }; m.prototype.getCircum = function () { return 360 }; m.prototype.toJSON = function () { return "{" + (this.wkid ? " wkid:" + this.wkid : "wkt: '" + this.wkt + "'") + "}" }; T.prototype = new m; w.prototype = new m; w.prototype.calc_m_ = function (a, b) {
        ...
        if (a && a.bounds) {
            var d = {}; d.f = a.f;
            var e = a.bounds; d.bbox = "" + e.getSouthWest().lng() + "," + e.getSouthWest().lat() + "," + e.getNorthEast().lng() + "," + e.getNorthEast().lat(); d.size = "" + a.width + "," + a.height; d.dpi = a.dpi; if (a.imageSR) d.imageSR = a.imageSR.wkid ? a.imageSR.wkid : "{wkt:" + a.imageSR.wkt + "}"; d.bboxSR = "4326"; d.format = a.format; e = a.layerDefinitions; if (e === undefined) e = this.getLayerDefs_(); d.layerDefs = U(e); e = a.layerIds; var f = a.layerOption || "show"; if (e === undefined) e = this.getVisibleLayerIds_(); if (e.length > 0) d.layers = f + ":" + e.join(","); else if (this.loaded_ &&
            b) { b({ href: null }); return } d.transparent = a.transparent === false ? false : true; if (a.time) d.time = ca(a.time, a.endTime); d.layerTimeOptions = a.layerTimeOptions; if (d.f === "image") return this.url + "/export?" + ia(d); else o(this.url + "/export", d, "", function (g) { if (g.extent) { g.bounds = W(g.extent); delete g.extent; b(g) } else v(c, g.error) })
        }
    }; n.prototype.identify = function (a, b, c) {
        if (a) {
            var d = {}; d.geometry = J(a.geometry); d.geometryType = S(a.geometry); d.mapExtent = J(a.bounds); d.tolerance = a.tolerance || 2; d.sr = 4326; d.imageDisplay = "" +
            a.width + "," + a.height + "," + (a.dpi || 96); d.layers = a.layerOption || "all"; if (a.layerIds) d.layers += ":" + a.layerIds.join(","); if (a.layerDefs) d.layerDefs = U(a.layerDefs); d.maxAllowableOffset = a.maxAllowableOffset; d.returnGeometry = a.returnGeometry === false ? false : true; o(this.url + "/identify", d, "", function (e) { var f, g, h; if (e.results) for (f = 0; f < e.results.length; f++) { g = e.results[f]; h = L(g.geometry, a.overlayOptions); g.feature = { geometry: h, attributes: g.attributes }; delete g.attributes } b(e); v(c, e) })
        }
    }; n.prototype.find = function (a,
    b, c) {
        if (a) {
            var d = l(a, {}); if (a.layerIds) { d.layers = a.layerIds.join(","); delete d.layerIds } if (a.searchFields) d.searchFields = a.searchFields.join(","); d.contains = a.contains === false ? false : true; if (a.layerDefinitions) { d.layerDefs = U(a.layerDefinitions); delete d.layerDefinitions } d.sr = 4326; d.returnGeometry = a.returnGeometry === false ? false : true; o(this.url + "/find", d, "", function (e) {
                var f, g, h; if (e.results) for (f = 0; f < e.results.length; f++) {
                    g = e.results[f]; h = L(g.geometry, a.overlayOptions); g.feature = { geometry: h, attributes: g.attributes };
                    delete g.attributes
                } b(e); v(c, e)
            })
        }
        ...
        var d = l(a, {}); if (d.inputs) { l(d.inputs, d); delete d.inputs } if (r(d.outFields)) d.outFields = d.outFields.join(","); var e = this; o(this.url + "/findAddressCandidates", d, "", function (f) {
            if (f.candidates) for (var g,
            h, j = [], C = 0; C < f.candidates.length; C++) { g = f.candidates[C]; h = g.location; if (!isNaN(h.x) && !isNaN(h.y)) { h = [h.x, h.y]; var D = e.spatialReference; if (a.outSR) D = B[a.outSR]; if (D) h = D.inverse(h); g.location = new i.LatLng(h[1], h[0]); j[j.length] = g } } b({ candidates: j }); v(c, f)
        })
    }; P.prototype.geocode = function (a, b) { this.findAddressCandidates(a, b) }; P.prototype.reverseGeocode = function (a, b, c) {
        if (!H(a.location)) a.location = J(a.location); a.outSR = 4326; var d = this; o(this.url + "/reverseGeocode", a, "", function (e) {
            if (e.location) {
                var f =
                e.location; if (!isNaN(f.x) && !isNaN(f.y)) { f = [f.x, f.y]; if (d.spatialReference) f = d.spatialReference.inverse(f); e.location = new i.LatLng(f[1], f[0]) }
            } b(e); v(c, e)
        })
    }; Z.prototype.project = function (a, b, c) { var d = ha(a); o(this.url + "/project", d, "callback", function (e) { var f = []; if (a.outSpatialReference === 4326 || a.outSpatialReference.wkid === 4326) { for (var g = 0, h = e.geometries.length; g < h; g++) f.push(L(e.geometries[g])); e.geometries = f } b(e); v(c, e) }) }; Z.prototype.buffer = function (a, b, c) {
        var d = ha(a); if (a.bufferSpatialReference) d.bufferSR =
        N(a.bufferSpatialReference); d.outSR = 4326; d.distances = a.distances.join(","); if (a.unit) d.unit = a.unit; o(this.url + "/buffer", d, "callback", function (e) { var f = []; if (e.geometries) for (var g = 0, h = e.geometries.length; g < h; g++) f.push(L(e.geometries[g], a.overlayOptions)); e.geometries = f; b(e); v(c, e) })
    }; ja.prototype.execute = function (a, b, c) {
        var d = {}; a.parameters && l(a.parameters, d); d["env:outSR"] = a.outSpatialReference ? N(a.outSpatialReference) : 4326; if (a.processSpatialReference) d["env:processSR"] = N(a.processSpatialReference);
        o(this.url + "/execute", d, "", function (e) { if (e.results) for (var f, g, h = 0; h < e.results.length; h++) { f = e.results[h]; if (f.dataType === "GPFeatureRecordSetLayer") for (var j = 0, C = f.value.features.length; j < C; j++) { g = f.value.features[j]; if (g.geometry) g.geometry = L(g.geometry, a.overlayOptions) } } b(e); v(c, e) })
    }; ka.prototype.solve = function (a, b, c) {
        if (a) {
            var d = l(a, {}); if (r(a.stops)) d.stops = ga(a.stops); if (r(a.barriers)) if (a.barriers.length > 0) d.barriers = ga(a.barriers); else delete d.barriers; d.returnRoutes = a.returnRoutes === false ?
            false : true; d.returnDirections = a.returnDirections === true ? true : false; d.returnBarriers = a.returnBarriers === true ? true : false; d.returnStops = a.returnStops === true ? true : false; o(this.url + "/solve", d, "", function (e) { e.routes && fa(e.routes.features, a.overlayOptions); b(e); v(c, e) })
        }
    }; t.prototype.fromLatLngToPoint = function (a, b) {
        if (!a || isNaN(a.lat()) || isNaN(a.lng())) return null; var c = this.spatialReference_.forward([a.lng(), a.lat()]), d = b || new i.Point(0, 0); d.x = (c[0] - this.originX_) / this.scale_; d.y = (this.originY_ - c[1]) /
        this.scale_; return d
        ...
        if (this.mapService_.tileInfo) {
            this.projection_ = new t(this.mapService_.tileInfo);
            this.minZoom = a.minZoom || this.projection_.minZoom; this.maxZoom = a.maxZoom || this.projection_.maxZoom
        }
        ...
        var c = b - (this.projection_ ? this.projection_.minZoom : this.minZoom), d = ""; if (!isNaN(a.x) && !isNaN(a.y) && c >= 0 && a.x >= 0 && a.y >= 0) {
            var e = this.mapService_.url; if (this.urlTemplate_) e = this.urlTemplate_.replace("[" + this.numOfHosts_ + "]", "" + (a.y + a.x) % this.numOfHosts_); d = this.projection_ || (this.map_ ? this.map_.getProjection() : t.WEB_MECATOR); if (!d instanceof t) d = t.WEB_MECATOR; var f =
            d.tileSize_, g = 1 << b, h = new i.Point(a.x * f.width / g, (a.y + 1) * f.height / g); g = new i.Point((a.x + 1) * f.width / g, a.y * f.height / g); h = new i.LatLngBounds(d.fromPointToLatLng(h), d.fromPointToLatLng(g)); g = this.mapService_.getFullBounds(); if (this.mapService_.singleFusedMapCache === false || b > this.dynaZoom) { c = { f: "image" }; c.bounds = h; c.format = "png32"; c.width = f.width; c.height = f.height; c.imageSR = d.spatialReference_; d = this.mapService_.exportMap(c) } else d = g && !g.intersects(h) ? "" : e + "/tile/" + c + "/" + a.y + "/" + a.x
        } return d
    }; A.prototype.setOpacity =
    function (a) { this.opacity_ = a; var b = this.tiles_; for (var c in b) b.hasOwnProperty(c) && I(b[c], a) }; A.prototype.getOpacity = function () { return this.opacity_ }; A.prototype.getMapService = function () { return this.mapService_ }; u.prototype.getTile = function (a, b, c) {
        for (var d = c.createElement("div"), e = "_" + a.x + "_" + a.y + "_" + b, f = 0; f < this.tileLayers_.length; f++) {
            var g = this.tileLayers_[f]; if (b <= g.maxZoom && b >= g.minZoom) {
                var h = g.getTileUrl(a, b); if (h) {
                    var j = c.createElement(document.all ? "img" : "div"); j.style.border = "0px none"; j.style.margin =
                    "0px"; j.style.padding = "0px"; j.style.overflow = "hidden"; j.style.position = "absolute"; j.style.top = "0px"; j.style.left = "0px"; j.style.width = "" + this.tileSize.width + "px"; j.style.height = "" + this.tileSize.height + "px"; if (document.all) j.src = h; else j.style.backgroundImage = "url(" + h + ")"; d.appendChild(j); g.tiles_[e] = j; if (g.opacity_ !== undefined) I(j, g.opacity_); else this.opacity_ !== undefined && I(j, this.opacity_)
                }
            }
        } this.tiles_[e] = d; d.setAttribute("tid", e); return d
    }; u.prototype.getTile = u.prototype.getTile; u.prototype.releaseTile =
    function (a) { if (a.getAttribute("tid")) { a = a.getAttribute("tid"); this.tiles_[a] && delete this.tiles_[a]; for (var b = 0; b < this.tileLayers_.length; b++) { var c = this.tileLayers_[b]; c.tiles_[a] && delete c.tiles_[a] } } }; u.prototype.releaseTile = u.prototype.releaseTile; u.prototype.setOpacity = function (a) { this.opacity_ = a; var b = this.tiles_; for (var c in b) if (b.hasOwnProperty(c)) for (var d = b[c].childNodes, e = 0; e < d.length; e++) I(d[e], a) }; u.prototype.getOpacity = function () { return this.opacity_ }; u.prototype.getTileLayers = function () { return this.tileLayers_ };
    p.prototype = new i.OverlayView; p.prototype.onAdd = function () { var a = this; this.listeners_.push(i.event.addListener(this.getMap(), "bounds_changed", oa(this.refresh, this))); this.listeners_.push(i.event.addListener(this.getMap(), "dragstart", function () { a.dragging = true })); this.listeners_.push(i.event.addListener(this.getMap(), "dragend", function () { a.dragging = false })); var b = this.getMap(); b.agsOverlays = b.agsOverlays || new i.MVCArray; b.agsOverlays.push(this); M(b); this.map_ = b }; p.prototype.onAdd = p.prototype.onAdd;
    p.prototype.onRemove = function () { for (var a = 0, b = this.listeners_.length; a < b; a++) i.event.removeListener(this.listeners_[a]); this.overlay_ && this.overlay_.setMap(null); b = this.map_; var c = b.agsOverlays; if (c) { a = 0; for (var d = c.getLength() ; a < d; a++) if (c.getAt(a) == this) { c.removeAt(a); break } } M(b); this.map_ = null }; p.prototype.onRemove = p.prototype.onRemove; p.prototype.draw = function () { if (!this.drawing_ || this.needsNewRefresh_ === true) this.refresh() }; p.prototype.draw = p.prototype.draw; p.prototype.getOpacity = function () { return this.opacity_ };
    p.prototype.setOpacity = function (a) { this.opacity_ = a = Math.min(Math.max(a, 0), 1); this.overlay_ && I(this.overlay_.div_, a) }; p.prototype.getMapService = function () { return this.mapService_ }; p.prototype.refresh = function () {
        if (this.drawing_ === true) this.needsNewRefresh_ = true; else {
            var a = this.getMap(), b = a ? a.getBounds() : null; if (b) {
                var c = this.exportOptions_; c.bounds = b; b = Q; var d = a.getDiv(); c.width = d.offsetWidth; c.height = d.offsetHeight; if (!(d.offsetWidth == 0 || d.offsetHeight == 0)) {
                    if ((a = a.getProjection()) && a instanceof t) b =
                    a.spatialReference_; c.imageSR = b; y(this, "drawstart"); var e = this; this.drawing_ = true; if (!this.dragging && this.overlay_) { this.overlay_.setMap(null); this.overlay_ = null } this.mapService_.exportMap(c, function (f) { e.drawing_ = false; if (e.needsNewRefresh_ === true) { e.needsNewRefresh_ = false; e.refresh() } else { if (f.href) { if (e.overlay_) { e.overlay_.setMap(null); e.overlay_ = null } e.overlay_ = new R(f.bounds, f.href, e.map_, e.opacity_) } y(e, "drawend") } })
                }
            }
        }
    }; p.prototype.isHidden = function () { return !(this.visible_ && this.isInZoomRange_()) };
    p.prototype.isInZoomRange_ = function () { var a = this.getMap().getZoom(); if (this.minZoom !== undefined && a < this.minZoom || this.maxZoom !== undefined && a > this.maxZoom) return false; return true }; p.prototype.show = function () { this.visible_ = true; this.div_.style.visibility = "visible"; this.refresh() }; p.prototype.hide = function () { this.visible_ = false; this.div_.style.visibility = "hidden" }; R.prototype = new i.OverlayView; R.prototype.onAdd = function () {
        var a = document.createElement("DIV"); a.style.border = "none"; a.style.borderWidth =
        "0px"; a.style.position = "absolute"; a.style.backgroundImage = "url(" + this.url_ + ")"; this.div_ = a; var b = this.getPanes(); I(a, this.op_); b.overlayLayer.appendChild(a)
    }; R.prototype.draw = function () { var a = this.getProjection(), b = a.fromLatLngToDivPixel(this.bounds_.getSouthWest()); a = a.fromLatLngToDivPixel(this.bounds_.getNorthEast()); var c = this.div_; c.style.left = b.x + "px"; c.style.top = a.y + "px"; c.style.width = a.x - b.x + "px"; c.style.height = b.y - a.y + "px" }; R.prototype.onRemove = function () {
        this.div_.parentNode.removeChild(this.div_);
        this.div_ = null
    }; la.prototype.refresh = function () { M(this.map_) }; $.ags = {
        SpatialReference: m, Geographic: T, LambertConformalConic: w, SphereMercator: F, TransverseMercator: E, SpatialRelationship: { INTERSECTS: "esriSpatialRelIntersects", CONTAINS: "esriSpatialRelContains", CROSSES: "esriSpatialRelCrosses", ENVELOPE_INTERSECTS: "esriSpatialRelEnvelopeIntersects", INDEX_INTERSECTS: "esriSpatialRelIndexIntersects", OVERLAPS: "esriSpatialRelOverlaps", TOUCHES: "esriSpatialRelTouches", WITHIN: "esriSpatialRelWithin" }, GeometryType: s,
        SRUnit: { METER: 9001, FOOT: 9002, SURVEY_FOOT: 9003, SURVEY_MILE: 9035, KILLOMETER: 9036, RADIAN: 9101, DEGREE: 9102 }, Catalog: function (a) { this.url = a; var b = this; o(a, {}, "", function (c) { l(c, b); y(b, "load") }) }, MapService: n, Layer: G, GeocodeService: P, GeometryService: Z, GPService: function (a) { this.url = a; this.loaded_ = false; var b = this; o(a, {}, "", function (c) { l(c, b); b.loaded_ = true; y(b, "load") }) }, GPTask: ja, RouteTask: ka, Util: z, Config: O, Projection: t, TileLayer: A, MapOverlay: p, MapType: u, CopyrightControl: la
    }; window.gmaps = $
})();