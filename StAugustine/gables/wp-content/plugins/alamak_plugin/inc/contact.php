...
function wpt_contact_posttype() {
	register_post_type( 'contact',
		array(
			'labels' => array(
				'name' => __( 'Contact Post' ),
				'singular_name' => __( 'Contact' ),
				'add_new' => __( 'Add New Contact' ),
				'add_new_item' => __( 'Add New Contact' ),
				'edit_item' => __( 'Edit Contact' ),
				'new_item' => __( 'Add New Contact' ),
			...
			'menu_position' => 5,
			'register_meta_box_cb' => 'contact_metabox_add',
...
add_action( 'init', 'wpt_contact_posttype' );
...
add_action('admin_enqueue_scripts', 'contact_metabox');

//add Post Details metabox
add_action( 'add_meta_boxes', 'contact_metabox_add' );
function contact_metabox_add()
{
    add_meta_box( 'contact-details', 'Post Details', 'metabox_contact_form', 'contact', 'normal', 'high' );
}

function metabox_contact_form( $post )
{
	...
	$gmap_option = isset( $values['gmap_option'] ) ? esc_attr( $values['gmap_option'][0] ) : '';
	$gallery_sport = isset( $values['gallery_port'] ) ? esc_attr( $values['gallery_port'][0] ) : '';
    wp_nonce_field( 'contact_meta_box_nonce', 'meta_box_nonce' );

?>

<p id="gmap-option">
	<label for="gmap_option">Use Google Map in This Post</label><br/>
    <input type="radio" name="gmap_option" value="yes" <?php if ( $gmap_option == 'yes' ){ echo "checked"; } ?>><b>Yes</b><small> (Remember to choose <b>"Standard"</b> post format.)</small><br/>
	<input type="radio" id="gmap_option" name="gmap_option" value="no" <?php if ( $gmap_option == 'no' ){ echo "checked"; } ?>><b>No</b><small> (You can choose any post format here.)</small>
</p>
<p id="tab-title-name" >
    ...
    <small>Enter Title for "contact" tab here</small>
</p>
<p id="black-text" >
    ...
    <small>Enter your black text here. eg: Technic</small>
</p>
<p id="grey-text" >
    ...
    <small>Enter your grey text here. eg: Photoshop</small>
</p>
...
function contact_meta_box_save( $post_id )
{
 ...
 if( !isset( $_POST['meta_box_nonce'] ) || !wp_verify_nonce( $_POST['meta_box_nonce'], 'contact_meta_box_nonce' ) ) return;
 ...
 if( isset( $_POST['gmap_option'] ) )
 update_post_meta( $post_id, 'gmap_option', wp_kses( $_POST['gmap_option'], $allowed ) );
}
